...
Deno.test("maintenance fallback KV read validates and normalizes cached payloads", async () => {
  ...
  assertEquals(
    (await readMaintenanceFallback(
      { MAINTENANCE_FALLBACK_KV: blank.kv } as Env,
    ))?.message,
  ...
  for (
    const value of [
      null,
      "{}",
      JSON.stringify({ enabled: false, message: "x", updated_at: "now" }),
    ]
  ) {
    const invalid = createKv(value);
    assertEquals(
      await readMaintenanceFallback(
        { MAINTENANCE_FALLBACK_KV: invalid.kv } as Env,
      ),
      null,
      "invalid fallback",
    );
  ...
  assertEquals(
    await readMaintenanceFallback({} as Env),
    null,
    "missing KV binding",
  );
});
...
Deno.test("maintenance fallback KV writes and clears use the exact key and TTL", async () => {
  ...
  assertEquals(
    state.deletes,
    ["itemtraxx:maintenance_fallback:v1"],
    "conditional clear",
  );
  await clearMaintenanceFallbackIfPresent(env);
  assertEquals(
    ...
    1,
    "already-empty KV should not delete again",
  );
...
Deno.test("maintenance extraction preserves enabled, disabled, and default-message semantics", () => {
  ...
  assertEquals(
    extractMaintenanceFromStatusPayload({
      maintenance: { enabled: true, message: " active ", updated_at: "stamp" },
    }),
    { enabled: true, message: "active", updated_at: "stamp" },
    "enabled maintenance",
  );
  const defaulted = extractMaintenanceFromStatusPayload({
    maintenance: { enabled: true, message: "", updated_at: "" },
  });
  ...
  assert(
    typeof defaulted?.updated_at === "string" &&
      defaulted.updated_at.length > 0,
    "defaulted timestamp",
  );

  const disabled = extractMaintenanceFromStatusPayload({
...
Deno.test("status fallback caches active maintenance and applies it only to unhealthy responses", async () => {
  ...
  assertEquals(
    await applyMaintenanceFallbackToStatusPayload(env, 200, healthy),
    healthy,
    "healthy payload",
  );

  for (
    const [statusCode, payload] of [
  ...
  const active = {
    status: "ok",
    maintenance: { enabled: true, message: " Fresh ", updated_at: "new-stamp" },
  };
  assertEquals(
    await applyMaintenanceFallbackToStatusPayload(env, 200, active),
    active,
    "active upstream payload",
  );
  ...
  assertEquals(
    await applyMaintenanceFallbackToStatusPayload(env, 200, disabled),
    disabled,
    "disabled upstream payload",
  );
