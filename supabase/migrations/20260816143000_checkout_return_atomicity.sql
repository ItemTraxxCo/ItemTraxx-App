...
-- The online checkout path used to update items and write item_logs through two
-- independent service-role calls. Keep the caller-boundary checks in the Edge
-- Function, but make the state transition and immutable audit row one database
-- transaction with a row lock and the same operation-id contract as offline
-- replay.
...
declare
  ...
  operation_id text := btrim(coalesce(p_operation_id, ''));
...
begin
  if p_workspace_id is null
     ...
     or operation_id = ''
     or char_length(operation_id) > 128
  ...
  if requested_action in ('checkout', 'return', 'auto') then
    select exists (
      ...
      where item_log.workspace_id = p_workspace_id
        ...
        and item_log.operation_id in (
          operation_id || ':' || p_item_id::text || ':checkout',
          operation_id || ':' || p_item_id::text || ':return'
        )
    ...
    select exists (
      ...
      where item_log.workspace_id = p_workspace_id
        and item_log.item_id = p_item_id
        and item_log.operation_id = operation_id || ':' || p_item_id::text || ':' || requested_action
    ) into has_existing_operation;
  end if;
  ...
  log_operation_id := operation_id || ':' || p_item_id::text || ':' || resolved_action;
