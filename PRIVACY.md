...
Cloudflare for edge proxying, request filtering, DNS, Turnstile, and Worker log delivery; Vercel for frontend hosting,
analytics, and performance measurements; PostHog for consent-controlled analytics, Error Tracking, Logs,
and optional Replay; Intercom for support Messenger conversations for anonymous visitors and authenticated users;
...
If image attachments are supported in a support workflow, those attachments are used only for handling the submitted request and related support operations.
...
If ItemTraxx becomes aware of a security incident that may have compromised your personal information, we will notify affected individuals or account holders as required by applicable law, including California Civil Code §1798.29 and §1798.82. For individuals and account holders not covered by an executed Data Processing Addendum (DPA), we will provide notification without undue delay and no later than thirty (30) calendar days after becoming aware of the incident, unless a shorter period is required by applicable law or law-enforcement delay applies. Notification will be provided via email to the address associated with your account if your data has been suspected to be compromised, or by an alternative method if required by law or if email is not available. For customers operating under an executed DPA, incident-notification timelines are governed by that agreement, which sets a default of notifying the customer without undue delay and, unless a shorter period is required by applicable law or law-enforcement delay applies, no later than thirty (30) calendar days after becoming aware of the incident; the DPA controls where its terms differ from this general statement.
---
