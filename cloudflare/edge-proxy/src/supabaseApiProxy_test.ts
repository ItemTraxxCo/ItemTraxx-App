...
Deno.test("Data API proxy retries 401 with a refreshed cookie session and streams response bytes", async () => {
  ...
  globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (auth === "Bearer old-access") return new Response("unauthorized", { status: 401 });
  ...
  try {
    const request = new Request("https://edge.itemtraxx.com/rest/v1/items?select=id", {
      method: "POST",
      headers: {
    ...
    assertEquals(Array.from(new Uint8Array(await response.arrayBuffer())), [0, 255, 7], "streamed bytes");
    assertEquals(response.headers.get("x-upstream"), "kept", "upstream header");
    assertEquals(response.headers.get("x-request-id"), "request-1", "request ID");
  } finally {
