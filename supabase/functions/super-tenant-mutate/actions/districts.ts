...
export const enrichDistricts = async (
  ...
  const { data: tenantCounts, error: tenantCountsError } = await context
    .adminClient
    .from("tenants")
    .select("district_id")
  ...
  if (!tenantCountsError) {
    for (
      const row of (tenantCounts ?? []) as Array<{ district_id: string | null }>
    ) {
      if (!row.district_id) continue;
      countsByDistrict.set(
        row.district_id,
        (countsByDistrict.get(row.district_id) ?? 0) + 1,
      );
    }
  }
...
export const handleDistrictAction = async (context: SuperTenantContext) => {
  ...
  if (action === "list_districts") {
    const search = optionalText(payload.search, {
      maxLen: 120,
      transform: "lowercase",
    });
    ...
    const { data, error } = await adminClient
      .from("districts")
      .select(
        "id, name, slug, support_email, contact_name, is_active, created_at, subscription_plan, billing_status, renewal_date, billing_email, invoice_reference",
      )
    ...
    const enriched = await enrichDistricts(
      context,
      (data ?? []) as DistrictRow[],
    );
    if (!search) {
  ...
  if (action === "create_district") {
    ...
    const slug = requireText(
      normalizeDistrictSlug(optionalText(next.slug, { maxLen: 80 })),
      {
        maxLen: 63,
    ...
    const invoiceReference =
      optionalText(next.invoice_reference, { maxLen: 120 }) || null;
    if (
      subscriptionPlan != null && subscriptionPlan !== "" &&
      !isValidDistrictPlan(subscriptionPlan)
    ) {
      return jsonResponse(400, { error: "Invalid subscription plan." });
    }
    if (
      billingStatus != null && billingStatus !== "" &&
      !isValidDistrictBillingStatus(billingStatus)
    ) {
    ...
    const { data, error } = await adminClient
      ...
      .insert({
        ...
        subscription_plan: isValidDistrictPlan(subscriptionPlan)
          ? subscriptionPlan
          : null,
        billing_status: isValidDistrictBillingStatus(billingStatus)
          ? billingStatus
          : null,
    ...
    if (error || !data) {
      ...
      return jsonResponse(400, {
        error: describeDistrictWriteError(
    ...
    return jsonResponse(200, {
      data: (await enrichDistricts(context, [data as DistrictRow]))[0],
    });
  }
  ...
  if (action === "get_district_details") {
    ...
    if (
      tenantRowsResult.error &&
      !isMissingDistrictIdColumn(tenantRowsResult.error as PgError)
    ) {
      return jsonResponse(400, { error: "Unable to load district tenants." });
    }

    const tenants = await enrichTenants(
      context,
      (tenantRowsResult.data ?? []) as TenantRow[],
    );
    ...
    if (profileIds.length) {
      ...
      emailById = new Map(
        ((profileRows ?? []) as Array<
          { id: string; auth_email: string | null }
        >).map((row) => [
          row.id,
          row.auth_email,
        ]),
    ...
    const tenantMetrics =
      ((metricRows ?? []) as Array<Record<string, number | string>>).map(
        (row) => ({
          tenant_id: String(row.tenant_id ?? ""),
          tenant_name: tenants.find((tenant) =>
            tenant.id === String(row.tenant_id ?? "")
          )?.name ??
    ...
    const usage = tenantMetrics.reduce(
      (acc, item) => ({
        ...
        active_checkouts: acc.active_checkouts +
          Number(item.active_checkouts ?? 0),
        overdue_items: acc.overdue_items + Number(item.overdue_items ?? 0),
        transactions_7d: acc.transactions_7d +
          Number(item.transactions_7d ?? 0),
      }),
      {
        gear_total: 0,
        students_total: 0,
    ...
    const { data: supportRequestRows } = await adminClient
      ...
      .select(
        "id, requester_email, requester_name, subject, message, priority, status, created_at",
      )
      .eq("district_id", id)
      .order("created_at", { ascending: false })
      .limit(50);
    ...
    const traffic = {
      checkout_24h: (recentLogs ?? []).filter((row) =>
        row.action_type === "checkout"
      ).length,
      return_24h: (recentLogs ?? []).filter((row) =>
        row.action_type === "return"
      ).length,
      active_tenants_24h: new Set(
        (recentLogs ?? []).map((row) => row.tenant_id).filter(Boolean),
      ).size,
    ...
    const studentById = new Map(
      ((studentRows.data ?? []) as Array<
        { id: string; username: string | null; student_id: string | null }
      >).map(
        (row) => [row.id, row],
      ),
    );
    const recentEvents = (recentLogs ?? []).slice(0, 20).map((row) => ({
      ...
      gear_name: typeof row.gear_id === "string"
        ? gearById.get(row.gear_id)?.name ?? null
        : null,
      gear_barcode: typeof row.gear_id === "string"
        ? gearById.get(row.gear_id)?.barcode ?? null
        : null,
      student_username: typeof row.checked_out_by === "string"
        ? studentById.get(row.checked_out_by)?.username ?? null
        : null,
      student_id: typeof row.checked_out_by === "string"
        ...
        ? [
          {
            ...
            count: tenantMetrics.filter((item) =>
              item.active_checkouts > 0
            ).length,
          },
        ]
        : []),
      ...((supportRequestRows ?? []).filter((row) => row.status === "open")
          .length > 0
        ? [
          {
    ...
    return jsonResponse(200, {
      data: {
        district:
          (await enrichDistricts(context, [district as DistrictRow]))[0],
        ...
        tenant_metrics: tenantMetrics.sort((a, b) =>
          b.transactions_7d - a.transactions_7d
        ),
        traffic,
