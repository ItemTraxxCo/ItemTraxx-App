...
serve(async(req)=>{
  ...
  try{
    ...
    const url=Deno.env.get("ITX_SUPABASE_URL") ?? Deno.env.get("SUPABASE_URL");const publishable=Deno.env.get("ITX_PUBLISHABLE_KEY") ?? Deno.env.get("SUPABASE_ANON_KEY");const service=Deno.env.get("ITX_SECRET_KEY") ?? Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");if(!url||!publishable||!service)return json(500,{error:"Server misconfiguration"});
