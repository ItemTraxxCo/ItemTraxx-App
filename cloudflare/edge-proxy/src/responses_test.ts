...
Deno.test("response builders preserve JSON envelopes, request IDs, and supplied headers", async () => {
  ...
  assertEquals(
    error.headers.get("x-upstream"),
    "kept",
    "extra upstream header",
  );
  assertEquals(
    error.headers.get("access-control-allow-credentials"),
    "true",
    "CORS header",
  );
...
Deno.test("session rate-limit responses preserve status, messages, and retry guidance", async () => {
  const limited = buildSessionRateLimitError(
    "rate_limited",
    {},
    "request-limited",
  );
  ...
  const unavailable = buildSessionRateLimitError(
    "unavailable",
    {},
    "request-unavailable",
  );
  ...
  assertEquals(
    unavailable.headers.get("retry-after"),
    null,
    "unavailable retry header",
  );
  assertEquals(
