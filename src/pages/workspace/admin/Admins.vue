    ...
    <div class="admin-hero">
      <div class="admin-toolbar">
        <div class="page-nav-left">
          ...
          <RouterLink class="button-link" to="/admin/settings">Settings</RouterLink>
        </div>
      ...
      <p class="admin-hero-copy">
        Manage workspace admin accounts for this workspace. Only primary admins can invite new admins,
      ...
      <div class="admin-section-header">
        <div>
          ...
          <p class="admin-section-copy">
            You can view the workspace admin roster, but only the primary admin can create or change workspace admin accounts.
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="canManageAdmins" class="card admin-section-card">
      <div class="admin-section-header">
        <div>
          ...
          <p class="admin-section-copy">
            Invite a new admin for this workspace by email. They will receive a setup link with further instructions.
          </p>
        </div>
      </div>
      <form class="form" @submit.prevent="handleCreate">
        <label>
          Email
          <input v-model="createEmail" type="email" placeholder="admin@workspace.example" />
        </label>
    ...
    <div class="card admin-section-card">
      <div class="admin-section-header">
        <div>
          ...
          <p class="admin-section-copy">
            Review workspace admin accounts, primary admin status, and current account access.
          </p>
        </div>
      </div>
      <p v-if="isLoading" class="muted">Loading workspace admins...</p>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="admin in admins" :key="admin.id">
              ...
              <td>{{ admin.is_primary_admin ? 'Primary' : 'Workspace admin' }}</td>
            ...
            <tr v-if="admins.length === 0">
              <td :colspan="canManageAdmins ? 5 : 4" class="muted">No workspace admins found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="editModalVisible && editTarget" class="modal-backdrop" @click.self="closeEditModal">
      <div class="modal admin-manage-modal">
        ...
        <form class="form" @submit.prevent="saveEditEmail">
          <label>
            Admin email
            <input v-model="editEmail" type="email" placeholder="admin@workspace.example" />
          </label>
          <div class="form-actions">
...
const loadAdmins = async () => {
  ...
  try {
    ...
    error.value = toUserFacingErrorMessage(err, "Unable to load workspace admins.");
  } finally {
...
const handleCreate = async () => {
  ...
  try {
    ...
    createEmail.value = "";
    showToast("Invitation accepted", "If this email is eligible, a workspace admin invitation will be sent.");
    await loadAdmins();
  } catch (err) {
    showToast("Create failed", toUserFacingErrorMessage(err, "Unable to invite workspace admin."));
  } finally {
...
const saveEditEmail = async () => {
  ...
  try {
    ...
    showToast("Email updated", "Workspace admin email was updated.");
    closeEditModal();
  } catch (err) {
    showToast("Update failed", toUserFacingErrorMessage(err, "Unable to update workspace admin email."));
  } finally {
    isSaving.value = false;
  }
};
...
const toggleEditStatus = async () => {
  ...
  try {
    ...
    admins.value = admins.value.map((item) => (item.id === updated.id ? updated : item));
    showToast(updated.is_active ? "Admin enabled" : "Admin disabled", "Workspace admin status updated.");
    closeEditModal();
  } catch (err) {
    showToast("Status update failed", toUserFacingErrorMessage(err, "Unable to update workspace admin status."));
  } finally {
    isSaving.value = false;
  }
};
...
const sendEditReset = async () => {
  ...
  try {
    ...
    showToast("Reset sent", "Password reset email sent to the workspace admin.");
    closeEditModal();
  } catch (err) {
