-- SQL Editor-safe version of advisor_fk_covering_indexes.sql.
-- `CONCURRENTLY` is removed because the Supabase SQL editor runs inside a transaction block.
-- Apply during a low-traffic window because plain CREATE INDEX can take stronger locks.
...
create index if not exists idx_item_checked_out_by on public.items (checked_out_by);
create index if not exists idx_item_deleted_by on public.items (deleted_by);
create index if not exists idx_item_logs_item_id on public.item_logs (item_id);
create index if not exists idx_item_logs_performed_by on public.item_logs (performed_by);
create index if not exists idx_item_status_history_changed_by on public.item_status_history (changed_by);
create index if not exists idx_item_status_history_item_id on public.item_status_history (item_id);
create index if not exists idx_borrowers_deleted_by on public.borrowers (deleted_by);
