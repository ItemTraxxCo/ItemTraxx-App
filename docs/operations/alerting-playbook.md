...
- `schedule-watchdog` workflow failure
...
4. If `schedule-watchdog` fired, compare the last successful run times for `Async Job Worker`, `Synthetic Journeys`, `Deployment Health`, and `Supabase Backup` against their expected cadence.
5. Determine blast radius and trigger rollback if needed.
6. Post status update if user-facing impact exists.
