-- Keep Postgres and Supavisor network-restricted. Better Auth reaches only the
-- HTTPS Data API with a server-held service_role credential.
alter role authenticator set pgrst.db_schemas = 'public, graphql_public, better_auth';
notify pgrst, 'reload config';

revoke all on schema better_auth from public, anon, authenticated;
grant usage on schema better_auth to service_role;
grant select, insert, update, delete on all tables in schema better_auth to service_role;
alter default privileges in schema better_auth revoke all on tables from public, anon, authenticated;
alter default privileges in schema better_auth grant select, insert, update, delete on tables to service_role;
...
begin
  ...
  if p_workspace_id is not null then
    ...
    from public.workspaces where id = p_workspace_id and deleted_at is null;
